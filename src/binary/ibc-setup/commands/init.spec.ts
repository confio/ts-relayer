...
test.serial('create app.yaml', async (t) => {
...
test.serial(
  'initialize home directory, pull registry.yaml and create app.yaml',
  async (t) => {
    const logger = new TestLogger();
...
test.serial('throws when cannot fetch registry.yaml from remote', async (t) => {
  const logger = new TestLogger();
...
test.serial('returns early if app.yaml exists', async (t) => {
  const logger = new TestLogger();
...
test.serial(
  'throws if provided chain does not exist in the registry',
  async (t) => {
    const logger = new TestLogger();
